...
claudex는 Claude Code를 재사용하기 때문에 **Anthropic 호환 엔드포인트(`/v1/messages`)를 제공하는 프로바이더만** 지원한다. 별도 SDK나 변환 로직은 없다. 각 프로바이더의 정확한 URL/모델 ID는 자주 바뀌므로 `claudex.config.example.json` 업데이트 시 공식 문서를 다시 확인할 것.
...
**ChatGPT Plus / Codex 구독 계정은 claudex에서 직접 사용할 수 없다.** claudex는 OAuth 플로우를 수행하지 않고 정적 토큰만 주입한다. OpenAI는 Anthropic 호환 엔드포인트를 제공하지 않기 때문에 브릿지 프록시가 필요하다. 실사용 시 패턴:
...
2. claudex 프로파일의 `baseUrl`을 그 로컬 URL로 지정

프록시 쪽에서 인증이 끝나므로 claudex의 `authToken`은 더미로도 충분하다.
