...
**세션 id는 대화 단위로 좁힌다**(`scopeSessionToConversation` + `ConversationKey`). 런치 단위 id 하나를 메인 대화와 서브에이전트가 공유하면, 먼저 도착한 쪽이 그 세션을 차지하고 자기 히스토리를 committed 스트림에 커밋해 다른 쪽 프리픽스가 어긋난다(실측: 서브에이전트가 메인 id를 차지한 턴 재사용률 38.2%). `ccx-<런치id>-<대화해시>` 형태로 발급하면 대화마다 세션이 갈려 서로 오염되지 않고, 동시 요청이 같은 id로 겹칠 일도 없어진다.

해시 재료는 **시스템 블록과 첫 메시지의 텍스트만**이다. Claude Code는 턴마다 `cache_control` 마커를 옮기고 content를 문자열↔블록배열로 바꾸므로, 그런 표현 차이까지 넣으면 같은 대화가 턴마다 다른 id를 받는다. 대화를 식별할 수 없으면 런치 id를 그대로 둔다 — 좁히지 못할 뿐 동작은 한다.

서브에이전트 2개 병렬 실행 실측(2026-09): 대화 3개가 각자 id를 유지, 409 0건, 메인 대화가 id를 뺏기지 않고 마지막 턴까지 프리픽스 재사용, 서브에이전트는 히스토리가 쌓이자 99.3%→99.7%.
